    ...
    return (
        <div className="max-w-[1700px] mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-8 py-9">
                {
                    load ? <div className="md:h-90 md:w-[1700px] flex justify-center items-center"><h1><MoonLoader color="blue"/></h1></div> : changeAppData.length === 0 ? <NoAppsFound/> : changeAppData.map(appData => (<Card key={appData.id} appData={appData}/>))
                }
            </div>
        </div>
    );
